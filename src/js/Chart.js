...
export class Chart {
  ...
  init() {
    ...
    if (this.data.length > 1) {
      const minDate = this.data[0].datetime;
      const maxDate = this.data[this.data.length - 1].datetime;
      ...
      for (let i = 0; i < this.data.length; i++) {
        ...
        const score = parseInt(this.data[i].score);
  ...
  draw() {
    ...
    if (this.data.length > 1) {
      ...
      for (let index = 0; index < this.data.length; index++) {
        ...
        const y = this.getYCoordinate(index);
        // draw line
        this.c.lineTo(x, y);
      }
      this.c.stroke();
  ...
  getXCoordinate(index) {
    const min = this.data[0].datetime;
    const max = this.data[index].datetime;
    const partBandwidth = max - min;
